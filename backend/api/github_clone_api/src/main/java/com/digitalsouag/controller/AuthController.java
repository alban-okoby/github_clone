...
public class AuthController {
	...
	public ResponseEntity<?> confirmRegistration(@NotEmpty @RequestParam String token) {
		VerificationToken tokenValue = this.tokenRepository.findByToken(token);

		if (tokenValue != null) {
			userService.validateVerificationToken(token);
			return ResponseEntity.ok().body("Your CvSKilling Account is activated successfully ✅ Now you can login.");
		}
		else {
